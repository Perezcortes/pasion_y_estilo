...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (!estadosPermitidos.includes(estado)) {
      return new NextResponse(JSON.stringify({ error: 'Estado no válido' }), {
        status: 400,
        headers: {
          'Content-Type': 'application/json',
          'Access-Control-Allow-Origin': '*'
    ...
    // Si es barbero, verificar que la cita es suya
    if (user.rol === 'BARBERO') {
      const [barbero]: any = await db.query(
        `SELECT id FROM barberos WHERE id_usuario = ?`,
        [user.id]
      ...
      if (barbero.length === 0 || barbero[0].id !== cita[0].id_barbero) {
        return new NextResponse(JSON.stringify({ error: 'No tienes permiso para modificar esta cita' }), {
          status: 403,
          headers: {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*'
    ...
    console.error('[ERROR_PUT_CITAS]', error)
    return new NextResponse(JSON.stringify({ error: 'Error al actualizar el estado de la cita' }), {
      status: 500,
      headers: {
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': '*'
